...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    affine_out, affine_cache = temporal_affine_forward(h, W_vocab, b_vocab)
  ...
  def sample(self, features, max_length=30):
    ...
    """
    N = features.shape[0]
    H = self.params['Wx'].shape[1] / {'lstm': 4, 'rnn': 1}[self.cell_type]
    ...
    c_in = self._start * np.ones((N, 1)).astype('int')
    c = np.zeros((N, H))
    for t in range(max_length):
        x, we_cache = word_embedding_forward(c_in, W_embed)
        if self.cell_type == 'rnn':
            next_h, _ = rnn_step_forward(np.squeeze(x), prev_h, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            next_h, c, _ = lstm_step_forward(np.squeeze(x), prev_h, c, Wx, Wh, b)
        x_temp, _ = temporal_affine_forward(next_h[:, np.newaxis, :], W_vocab, b_vocab)
        ix = np.squeeze(np.argmax(x_temp, axis=2))
        captions[:, t] = ix
        c_in = captions[:, t]
        prev_h = next_h
