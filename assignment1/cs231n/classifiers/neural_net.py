...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss =  np.sum(-np.log(scores_exp[indexs, y] / np.sum(scores_exp, 1)))
    loss /= N
    ...
    #############################################################################
    grad_scores = (1/np.sum(scores_exp, axis=1).reshape(N, -1))* scores_exp
    ...
    grads['W2'] = np.dot(h1.T, grad_scores)
    grads['W2'] /= N
    grads['W2'] += reg*W2
    
    dh1 = np.dot(grad_scores, W2.T)
    dz1 = dh1
    dz1[h1==0] = 0
